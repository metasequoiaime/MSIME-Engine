...
std::size_t write(void *data, std::size_t size, std::size_t count, void *context) noexcept
{
    ...
    if (length > limit - body.size())
        return 0;
    try
    {
        body.append(static_cast<const char *>(data), length);
    }
    catch (...)
    {
        return 0;
...
std::string request(const RequestOptions &options, const std::function<std::shared_ptr<void>(CURL *)> &configure)
{
    ...
    if (options.token.find_first_of("\r\n") != std::string::npos)
        throw VoiceError("Invalid voice token");
    ...
    if (result != CURLE_OK)
        throw VoiceError(std::string("Voice HTTP request failed: ") + curl_easy_strerror(result));
    long status = 0;
    curl_easy_getinfo(curl, CURLINFO_RESPONSE_CODE, &status);
    if (status < 200 || status >= 300)
        throw VoiceError("Voice HTTP status " + std::to_string(status));
    if (options.cancelled && options.cancelled->load())
        throw VoiceError("Voice request cancelled");
