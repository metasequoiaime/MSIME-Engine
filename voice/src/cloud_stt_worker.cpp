...
CloudSttWorker::CloudSttWorker(const std::string &token) : CloudSttWorker(RequestOptions{"https://api.siliconflow.cn/v1/audio/transcriptions", "TeleAI/TeleSpeechASR", token, 10000, {}})
{
}
std::string CloudSttWorker::recognize(const std::vector<float> &pcm)
{
    ...
    const auto payload = make_transcription_request(std::string_view(reinterpret_cast<const char *>(wav.data()), wav.size()), options_.model);
    const auto response = detail::request(options_, [&](CURL *curl) -> std::shared_ptr<void> {
        ...
        if (!headers)
            throw VoiceError("Cannot create transcription headers");
        auto *next = curl_slist_append(headers.get(), ("Content-Type: " + payload.content_type).c_str());
        if (!next)
            throw VoiceError("Cannot create transcription content type");
