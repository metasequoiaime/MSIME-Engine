...
{
    if (offset > bytes.size() || bytes.size() - offset < 4) invalid();
...
Bytes compress(const Bytes &input)
{
    ...
    if (deflateInit2(&stream, Z_DEFAULT_COMPRESSION, Z_DEFLATED, MAX_WBITS + 16, 8,
                     Z_DEFAULT_STRATEGY) != Z_OK) invalid();
    ...
    deflateEnd(&stream);
    if (status != Z_STREAM_END) invalid();
    output.resize(size);
    return output;
}
Bytes decompress(const Bytes &input)
{
    ...
    z_stream stream{};
    if (inflateInit2(&stream, MAX_WBITS + 16) != Z_OK) invalid();
    stream.next_in = const_cast<Bytef *>(input.data());
    stream.avail_in = static_cast<uInt>(input.size());
    stream.next_out = output.data();
    stream.avail_out = static_cast<uInt>(output.size());
    ...
    inflateEnd(&stream);
    if (status != Z_STREAM_END || remaining || size > doubao_response_limit) invalid();
    output.resize(size);
    return output;
}
...
{
    if (!body.is_object() || !body.contains("result")) return {};
    const auto &result = body["result"];
    auto text = [](const nlohmann::json &segment) -> std::string {
        if (!segment.is_object() || !segment.contains("text")) return {};
        if (!segment["text"].is_string()) invalid();
        return segment["text"].get<std::string>();
    };
    if (result.is_object()) return text(result);
    if (!result.is_array()) invalid();
    std::string value;
    for (const auto &segment : result) value += text(segment);
    return value;
}
} // namespace
std::vector<std::uint8_t> make_doubao_request(const DoubaoRequestOptions &options)
{
    if (options.boosting_table_id.size() > 8192) invalid();
    try
    {
        nlohmann::json request = {{"model_name", "bigmodel"}, {"enable_itn", options.enable_itn},
            {"enable_punc", options.enable_punc}, {"enable_ddc", options.enable_ddc},
            {"show_utterances", false}, {"result_type", "full"}};
        if (!options.boosting_table_id.empty()) request["corpus"] = {{"boosting_table_id", options.boosting_table_id}};
        const auto json = nlohmann::json{{"user", {{"uid", "metasequoia-ime"}}},
            {"audio", {{"format", "pcm"}, {"codec", "raw"}, {"rate", 16000}, {"bits", 16}, {"channel", 1}}},
            {"request", std::move(request)}}.dump();
...
{
    if (sequence < 2 || count > doubao_chunk_samples || (!samples && count) || (!last && !count)) invalid();
    Bytes pcm;
    pcm.reserve(count * 2);
    for (std::size_t i = 0; i < count; ++i)
    {
        if (!std::isfinite(samples[i]) || std::fabs(samples[i]) > 1) invalid();
...
{
    if (message.size() < 8 || message.size() > doubao_response_limit || (message[0] >> 4) != 1) invalid();
    ...
    if (offset < 4 || offset > message.size() || flags > 7 || compression > 1 ||
        (type != 9 && type != 15) || (serialization != 1 && type != 15)) invalid();
    if (flags & 1) (void)read32(message, offset);
    if (flags & 4) (void)read32(message, offset);
    DoubaoResponse result;
    result.last = (flags & 2) != 0;
    if (type == 15) result.code = read32(message, offset);
    const auto size = read32(message, offset);
    if (size != message.size() - offset) invalid();
    Bytes payload(message.begin() + static_cast<std::ptrdiff_t>(offset), message.end());
    if (compression) payload = decompress(payload);
    ...
    try
    {
        ...
        if (!json.is_object()) invalid();
        result.text = transcript(json);
        if (result.text.empty() && json.contains("payload_msg")) result.text = transcript(json["payload_msg"]);
        return result;
    }
    catch (const nlohmann::json::exception &) { invalid(); }
}
