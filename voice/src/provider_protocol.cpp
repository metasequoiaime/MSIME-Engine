...
nlohmann::json parse_response(std::string_view response) {
    if (response.size() > maximum_response_bytes) throw VoiceError("Voice response is too large");
    try { return nlohmann::json::parse(response.begin(), response.end()); }
    catch (const nlohmann::json::exception&) { throw VoiceError("Invalid voice response"); }
}
...
MultipartRequest make_transcription_request(std::string_view wav, std::string_view model,
    ...
    std::string boundary;
    do { boundary = "----MetasequoiaVoice" + std::to_string(sequence.fetch_add(1)); }
    while (wav.find(boundary) != std::string_view::npos || model.find(boundary) != std::string_view::npos ||
           language.find(boundary) != std::string_view::npos);
...
std::string make_polish_request(std::string_view model, std::string_view system_prompt,
    ...
    try {
        return nlohmann::json{
            {"model", std::string(model)}, {"stream", false},
            {"messages", {{{"role", "system"}, {"content", std::string(system_prompt)}},
                          {{"role", "user"}, {"content", std::string(user_message)}}}}
        }.dump();
    } catch (const nlohmann::json::exception&) { throw VoiceError("Invalid polish request text"); }
}
std::string parse_polished_text(std::string_view response) {
    ...
    try {
        ...
        if (!text.empty()) return text;
    } catch (const nlohmann::json::exception&) { throw VoiceError("Missing polished text"); }
