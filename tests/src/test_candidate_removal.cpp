...
void type(Session &session, const std::string &input) { for (char c : input) session.character(c); }
bool has(const Session &session, const std::string &word)
{
    const auto view = session.snapshot();
    return std::any_of(view.candidates.begin(), view.candidates.end(), [&](const auto &item) { return item.word == word; });
}
...
int main()
{
    ...
    require(EnglishDictionary::ensure_schema(path_to_utf8(resources / assets::english_dictionary)), "English schema failed");
    ...
    for (const auto scheme : {SchemeType::Quanpin, SchemeType::Shuangpin, SchemeType::Wubi})
    {
        ...
        options.paths = prepare_runtime_paths(resources, user, cache, "v2");
        {
            ...
            type(replayed, input);
            require(!has(replayed, "拟好") && has(replayed, "你好"), "Removed phrase reappeared after upgrade replay");
        }
        options.paths = prepare_runtime_paths(resources, user, cache, "v1");
    ...
    for (const int mode : {0, 1, 2})
    {
        ...
        {
            Session session(options);
            if (mode == 0) session.set_dedicated_english(true);
            else if (mode == 1) session.character('Y', true);
            type(session, "he");
    ...
    execute(journal,
        "CREATE TRIGGER reject_removal BEFORE INSERT ON user_dictionary_operations "
        "BEGIN SELECT RAISE(ABORT,'fixture rejection'); END;");
    {
        ...
        require(result.handled && result.diagnostic && !result.commit && has(session, "拟好") &&
                session.snapshot().preedit == before, "Failed journal write lost composition");
        require(count(failure_options.paths.dictionary(assets::main_dictionary),
            "SELECT count(*) FROM tbl_2_n WHERE value='拟好'") == 1, "Journal failure did not roll back deletion");
        ...
        require(count(failure_options.paths.dictionary(assets::english_dictionary),
            "SELECT count(*) FROM english_words WHERE word='help'") == 1, "English deletion was not rolled back");
    }
    // The legacy English caller uses the same rollback guarantee.
    require(!user_dictionary::delete_english_candidate(
        path_to_utf8(failure_options.paths.dictionary(assets::english_dictionary)), path_to_utf8(journal), "help", "help"),
        "Legacy English deletion concealed journal failure");
    require(count(failure_options.paths.dictionary(assets::english_dictionary),
        "SELECT count(*) FROM english_words WHERE word='help'") == 1, "Legacy English deletion lost an unjournaled row");
    ...
    require(user_dictionary::delete_dictionary_candidate(
        ...
        user_dictionary::DictionaryKind::Pinyin, "xi'an", "西安"), "Canonical deletion failed");
    require(count(failure_options.paths.dictionary(assets::main_dictionary),
    ...
    require(count(journal, "SELECT count(*) FROM user_dictionary_operations WHERE operation='delete' AND key='xi''an'") == 1,
        "Deletion journal lost canonical boundaries");
    require(count(journal, "SELECT count(*) FROM user_dictionary_operations") == 1, "Failed removal left extra tombstones");
    require(main_before == bytes(resources / assets::main_dictionary) &&
            english_before == bytes(resources / assets::english_dictionary), "Removal modified immutable resources");
}
