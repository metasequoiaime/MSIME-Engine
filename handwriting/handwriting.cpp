...
{
    std::unique_ptr<zinnia::Recognizer> engine{zinnia::Recognizer::create()};
};
Recognizer::Recognizer(const std::string &path) : impl_(std::make_unique<Impl>())
{
    if (!impl_->engine->open(path.c_str()))
        throw std::runtime_error("Cannot open handwriting model");
}
Recognizer::~Recognizer() = default;
std::vector<std::string> Recognizer::recognize(const std::vector<Stroke> &strokes, float width, float height)
{
    if (!std::isfinite(width) || !std::isfinite(height) || width <= 0 || height <= 0 || width > 10000 ||
        height > 10000 || strokes.size() > 64)
    ...
    {
        if (strokes[i].empty() || strokes[i].size() > 512)
            throw std::invalid_argument("Invalid handwriting stroke");
        for (auto point : strokes[i])
        {
            if (!std::isfinite(point.x) || !std::isfinite(point.y) || point.x < 0 || point.y < 0 || point.x > width ||
                point.y > height)
