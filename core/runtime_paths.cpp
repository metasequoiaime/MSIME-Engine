...
{
    if (root.empty() || name.empty() || name.is_absolute()) return {};
    for (const auto &part : name)
        if (part == "..") throw std::invalid_argument("Runtime asset path escapes its directory");
    return root / name;
}
...
void copy_database(const std::filesystem::path &source, const std::filesystem::path &target)
{
    ...
    const bool opened = sqlite3_open_v2(path_to_utf8(source).c_str(), &input, SQLITE_OPEN_READONLY, nullptr) == SQLITE_OK &&
        sqlite3_open_v2(path_to_utf8(target).c_str(), &output, SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE, nullptr) == SQLITE_OK;
...
std::filesystem::path RuntimePaths::resource(const std::filesystem::path &name) const { return join(resources, name); }
std::filesystem::path RuntimePaths::user(const std::filesystem::path &name) const { return join(user_data, name); }
std::filesystem::path RuntimePaths::dictionary(const std::filesystem::path &name) const { return join(dictionaries, name); }
void RuntimePaths::validate() const
{
    for (const auto &path : {resources, user_data, cache, dictionaries})
        if (!path.is_absolute()) throw std::invalid_argument("Runtime directories must be absolute");
}
                                   ...
                                   const std::string &content_id)
{
    ...
    try
    {
        ...
        const auto replay = user_dictionary::replay(path_to_utf8(result.user(assets::user_journal)),
            path_to_utf8(stage / assets::main_dictionary), path_to_utf8(stage / assets::english_dictionary));
        if (replay.failed || !replay.error.empty()) throw std::runtime_error("Runtime dictionary replay failed: " + replay.error);
        std::ofstream ready(stage / ".ready");
        ready << content_id << '\n';
        ready.close();
        if (!ready) throw std::runtime_error("Cannot finalize runtime dictionary generation");
