...
{
    if (dedicated_english_mode_) return dedicated_english_preedit_;
    if (local_input_mode_ == LocalInputMode::TemporaryJapanese)
        return "R" + engine_.get_request().raw_input_with_cases;
    if (local_input_mode_ != LocalInputMode::None) return local_preedit_;
...
KeyResult InputSession::edit_at_caret(Command command)
{
    ...
    switch (command)
    {
    case Command::MoveLeft: caret = caret > begin ? caret - 1 : begin; break;
    case Command::MoveRight: caret = std::min(caret + 1, text.size()); break;
    case Command::MoveHome: caret = begin; break;
    case Command::MoveEnd: caret = text.size(); break;
    case Command::Backspace:
        if (caret <= begin) return {true, std::nullopt, std::nullopt};
        text.erase(--caret, 1);
        return replace_editing_text(std::move(text), caret);
    case Command::DeleteForward:
        if (caret == text.size()) return {true, std::nullopt, std::nullopt};
        text.erase(caret, 1);
        return replace_editing_text(std::move(text), caret);
    default: return {};
    }
...
KeyResult InputSession::insert_at_caret(char character)
{
    ...
    if (!dedicated_english_mode_)
    {
        ...
        case LocalInputMode::Unicode:
            ...
            break;
        case LocalInputMode::QuickPhrase: accepted = lower; break;
        case LocalInputMode::DateTime: accepted = false; break;
        case LocalInputMode::None:
            ...
            if (character == '\'' && scheme() != SchemeType::Wubi) accepted = caret > 0;
            break;
