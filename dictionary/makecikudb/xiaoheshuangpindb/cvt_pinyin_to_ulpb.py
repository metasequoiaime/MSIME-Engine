...
single_chars_path = os.path.join(os.path.dirname(
    __file__), "../cn/SingleCharsAllV1.txt")
res_path = os.path.join(os.path.dirname(__file__),
                        "./cn/SingleCharsAll_Ul_V1.txt")

count = 0
with open(single_chars_path, "rb") as file:
    with open(res_path, "wb") as res_file:
        all_lines = file.readlines()[1:]  # 排除第一行的注释
        for line in all_lines:
            cur_line = line.decode().strip()
            cur_line_list = cur_line.split("\t")
            try:
                cur_line_to_write = cur_line_list[0] + "\t" + cvt_pinyin_to_ul(
